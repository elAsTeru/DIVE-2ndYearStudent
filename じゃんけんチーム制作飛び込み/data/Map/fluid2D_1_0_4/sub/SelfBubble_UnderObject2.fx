// パーティクル座標を保存するテクスチャ
shared texture2D ParticlePosTex_SB2 : RENDERCOLORTARGET;
sampler2D ParticlePosSamp = sampler_state {
	texture = <ParticlePosTex_SB2>;
	Filter = NONE;
	AddressU = CLAMP; AddressV = CLAMP;
};
// パーティクル速度と寿命を保存するテクスチャ
shared texture2D ParticleVecTex_SB2 : RENDERCOLORTARGET;
sampler2D ParticleVecSamp = sampler_state {
	texture = <ParticleVecTex_SB2>;
	Filter = NONE;
	AddressU = CLAMP; AddressV = CLAMP;
};
// 接触率と目標角度
shared texture2D ParticleNormTex_SB2 : RENDERCOLORTARGET;
sampler2D ParticleNormSamp = sampler_state {
	texture = <ParticleNormTex_SB2>;
	Filter = NONE;
	AddressU = CLAMP; AddressV = CLAMP;
};
...
/* ピクセルシェーダー */
PS_OUTPUT PtclMain_PS( PTCL_VS_OUTPUT IN) : COLOR0
{
    ...
    float3 Specular = pow( max(0,dot( HalfVector, normalize(normal) )), 16 );
	LastColor.rgb += Specular;
	LastColor *= nd;
	LastColor.rgb = 1;
	LastColor.a *= pow(1-saturate(dot(normalize(Eye),normal)),2);
	LastColor.a += Specular;
	LastColor.a = saturate(LastColor.a);
	...
	if(IN.indexF == 0) LastColor.a = 0;
	
	PS_OUTPUT Out = (PS_OUTPUT)0;
	Out.Normal = float4(normal,LastColor.a > 0.3);
	Out.Pos = float4(IN.WPos,LastColor.a > 0.3);
